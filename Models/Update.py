	...
	def train(self, net):
		...
		for iter in range(self.args.local_ep):
			...
			for batch_idx, (images, labels) in enumerate(self.ldr_train):
				...
				# compute the accuracy of batchsize samples
				_, preds = torch.max(log_probs, 1)  # 获取预测的标签
				correct = (preds == labels).sum().item()  # 计算正确预测的数量
				accuracy = correct / len(labels)  # 计算当前batch的准确度
				batch_acc.append(accuracy)  # 将准确度添加到batch_acc列表
			
			epoch_loss.append(sum(batch_loss) / len(batch_loss))
			epoch_acc.append(sum(batch_acc) / len(batch_acc))
		
		# return net.state_dict(), sum(epoch_loss) / len(epoch_loss), 100 * sum(epoch_acc) / len(epoch_acc)
