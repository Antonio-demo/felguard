		...
		if test_backdoor:
			...
			for k, image in enumerate(data):
				if test_or_not(args, target[k]):  # one2one need test
					...
					if args.dataset == 'mnist' or args.dataset == 'fmnist' or args.dataset == 'cifar' \
							or args.dataset == 'cifar100' or args.dataset == 'gtsrb':
						save_img(data[k])
...
def test_label(net_g, data_loader, args):
	# Compute the correct matrix of predicted labels and t-SNE figure
	net_g.eval()
	...
	with torch.no_grad():
		for data, target in data_loader:
			...
			flipped_target = np.where(target_cpu == 5, 7, target_cpu)
			flipped_target = np.where(target_cpu == 5, 10, flipped_target)
			
			true_labels.extend(target_cpu)
			pred_labels.extend(pred.cpu().numpy().squeeze())
			features.extend(output.cpu().numpy())
			flipped_labels.extend(flipped_target)
	
	return np.array(true_labels), np.array(pred_labels), np.array(features), np.array(flipped_labels)
