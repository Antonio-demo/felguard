...
class LocalMaliciousUpdate(object):
	def __init__(self, args, dataset=None, idxs=None, attack=None, order=None, malicious_list=None, dataset_test=None):
		...
		self.ldr_train = DataLoader(DatasetSplit(
			dataset, idxs), batch_size=self.args.local_bs, shuffle=False)
		#  change 0708
	...
	def train(self, net, test_img=None):
		...
		elif self.attack == 'biasattack':
			return self.train_malicious_biasattack(net)
		elif self.attack == 'labelflip':
			return self.train_malicious_labelflip(net)
		elif self.attack == 'biasattack':
			return self.train_malicious_biasattack(net)
		else:
	...
	def train_malicious_biasattack(self, net, test_img=None, dataset_test=None, args=None):
		...
		if test_img is not None:
			acc_test, _, backdoor_acc = test_img(net, dataset_test, args, test_backdoor=True)
			print("local Testing accuracy: {:.2f}".format(acc_test))
			print("local Backdoor accuracy: {:.2f}".format(backdoor_acc))
		return net.state_dict(), sum(epoch_loss) / len(epoch_loss)
