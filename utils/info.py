...
def information(args):
	...
	if args.defence == 'RLR':
		...
		if args.turn != 0:
			info.append('proportion of malicious are selected:' + str(
				args.wrong_mal / (int(args.malicious * max(int(args.frac * args.num_users), 1)) * args.turn)))
			info.append('proportion of benign are selected:' + str(args.right_ben / ((max(int(
				args.frac * args.num_users), 1) - int(
				args.malicious * max(int(args.frac * args.num_users), 1))) * args.turn)))
