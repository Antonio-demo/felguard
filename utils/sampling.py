...
def mnist_noniid_dirichlet(dataset, num_users, alpha=0.5):
	...
	:param alpha: Dirichlet distribution concentration parameter (controls heterogeneity)
	:return: dict_users: Dictionary of user indices
	"""
	# 初始化用户数据字典
	dict_users = {i: np.array([], dtype='int64') for i in range(num_users)}
	...
	labels = dataset.train_labels.numpy()  # MNIST 数据集标签
	
	# 按标签分组
	num_classes = len(np.unique(labels))  # 10 类
	idxs_labels = {i: idxs[labels == i] for i in range(num_classes)}
	...
	for label in idxs_labels:
		...
		# 将索引按照分布分配给用户
		proportions = (proportions * len(idxs_labels[label])).astype(int)
		
		# 防止分配不足
		while sum(proportions) < len(idxs_labels[label]):
			proportions[np.argmax(proportions)] += 1
		
		# 按照分布分配数据索引
		idxs_split = np.split(idxs_labels[label], np.cumsum(proportions[:-1]))
		for i, idx in enumerate(idxs_split):
			dict_users[i] = np.concatenate((dict_users[i], idx), axis=0)
	
	return dict_users


def gtsrb_noniid_dirichlet(dataset, num_users, alpha=0.9):
	"""
	Sample non-I.I.D client data from MNIST dataset using Dirichlet distribution.
	...
	idxs = np.arange(len(dataset))
	#labels = dataset.train_labels.numpy()  # MNIST 数据集标签
	labels = [data[1] for data in dataset]  # 假设每个样本为(data, label)
	labels = np.array(labels)
	
	# 按标签分组
	num_classes = len(np.unique(labels))  # 43 类
	idxs_labels = {i: idxs[labels == i] for i in range(num_classes)}
